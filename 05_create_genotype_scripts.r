...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/33_albertine"
	directory_name <- "11_genotype_scripts_alb"
	reference_genome_location <- "/home/jmanthey/references/FicAlb1.5__GCA_000247815.2.fna"
	cluster <- "quanah"
	output_name <- "alb_genotype"
	popmap <- "alb_popmap.txt"
	individuals <- read.table(popmap, sep="\t")
	faidx <- read.table("FicAlb1.5__GCA_000247815.2.fna.fai", sep="\t", stringsAsFactors=F)
	
	min_scaffold_size <- 2000000
	max_genotype_job_size <- 10000000
	max_individual_genotype_job_size <- 150000000
	...
	faidx_keep <- faidx[faidx[,2] < 150000000,1:2]
	faidx_change <- faidx[faidx[,2] >= 150000000,1:2]
