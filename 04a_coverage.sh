...
26_final.bam 27_final.bam 28_final.bam 29_final.bam 30_final.bam 31_final.bam 32_final.bam 33_final.bam 34_final.bam > \
alb_coverage.txt
...
while read -r name1 number1; do
	number2=$((number1 + 2));
  cut alb_coverage.txt -f $number2 > ${name1}_depth.txt;
done < alb_popmap.txt
